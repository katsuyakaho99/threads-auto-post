"""株式会社蒼蓮あて 2026年7月分 請求書を生成する。
交通費は提供された領収書・PASMO履歴から集計済み。"""
import os
import openpyxl
...
items = [
    ...
    (19, "7/22 交通費 新幹線（新大阪→新横浜）", 1, 14190),
    (20, "7/22 交通費 現地移動（PASMO：大阪→新大阪／新横浜→菊名→みなとみらい）", 1, 708),
    (21, "7/23 交通費 新幹線（新横浜→京都）※Six Senses Kyoto 研修・報酬なし", 1, 13300),
    (22, "7/23 交通費 現地移動（PASMO：みなとみらい→菊名→新横浜／京都→大阪／梅田→阿波座）", 1, 1348),
]
...
note = ("※ インボイス制度の経過措置（80%控除）に基づき、消費税10%ではなく調整額8%（小計×8%）で計上しています。\n"
        "※ Six Senses Kyoto（7/23）は研修のため報酬は発生していません（交通費のみ）。\n"
        "※ 8/4 Aloft の報酬・交通費は「2026年8月分」として別途ご請求予定です。\n"
...
out = os.path.join(BASE, "invoice_soren_2026-07.xlsx")
wb.save(out)
print("saved", out)
...
adj = round(sub * 0.08)
print("小計(想定):", sub, "調整額8%(想定):", adj, "合計(想定):", sub + adj)
for c in ["E26", "E27", "E28", "E30", "D18", "D19", "D20", "D21", "D22"]:
